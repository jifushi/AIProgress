...
func Login(username, email, password string) (string, common_code.Code) {
	...
	if username != "" {
		...
		// 4. 生成token
		token, err := utls_jwt.Create(userInformation.ID, userInformation.Username)
		if err != nil {
...
func Register(email, password, verification string) (string, common_code.Code) {
	...
	// 3. 生成用户名
	username := utls.CreateRandomNumber(11)
	password = utls.MD5(password)
	// 4. 添加用户到数据库
...
func SendVerificationEmail(email string) common_code.Code {
	// 1. 检查邮箱是否已经注册
	verification := utls.CreateRandomNumber(6)
